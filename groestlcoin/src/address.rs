...
pub enum Error {
    ...
    /// Groestlcoin script opcode does not match any known witness version, the script is malformed.
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            Error::UnknownAddressType(ref s) => write!(f, "unknown address type: '{}' is either invalid or not supported in rust-groestlcoin", s),
...
impl WitnessVersion {
    ...
    /// version in groestlcoin script. Thus, there is no function to directly convert witness version
    /// into a byte since the conversion requires context (groestlcoin script or just a version number).
...
impl TryFrom<opcodes::All> for WitnessVersion {
    ...
    /// Converts groestlcoin script opcode into [`WitnessVersion`] variant.
    ///
    /// # Returns
...
impl<'a> TryFrom<Instruction<'a>> for WitnessVersion {
    ...
    /// Converts groestlcoin script [`Instruction`] (parsed opcode) into [`WitnessVersion`] variant.
    ///
    /// # Returns
...
impl From<WitnessVersion> for opcodes::All {
    /// Converts [`WitnessVersion`] instance into corresponding Groestlcoin scriptopcode (`OP_0`..`OP_16`).
...
pub struct AddressEncoding<'a> {
    ...
    /// base58 version byte for p2pkh payloads (e.g. 0x24 for "F..." addresses).
    pub p2pkh_prefix: u8,
...
/// use groestlcoin::address::{NetworkUnchecked, NetworkChecked};
///
/// // variant 1
/// let address: Address<NetworkUnchecked> = "32iVBEu4dxkUQk9dJbZUiBiQdmypaPrkNU".parse().unwrap();
/// let address: Address<NetworkChecked> = address.require_network(Network::Groestlcoin).unwrap();
///
/// // variant 2
/// let address: Address = Address::from_str("32iVBEu4dxkUQk9dJbZUiBiQdmypaPrkNU").unwrap()
///                .require_network(Network::Groestlcoin).unwrap();
///
/// // variant 3
/// let address: Address<NetworkChecked> = "32iVBEu4dxkUQk9dJbZUiBiQdmypaPrkNU".parse::<Address<_>>()
///                .unwrap().require_network(Network::Groestlcoin).unwrap();
/// ```
///
...
/// let address: Address<NetworkChecked> = Address::from_str("FXBxTzaqSQsBFkL7DSAkqbaEvhN4HxNsAr")
...
/// ```
/// # use std::str::FromStr;
/// # use groestlcoin::address::{Address, NetworkUnchecked};
/// let address: Address<NetworkUnchecked> = Address::from_str("FXBxTzaqSQsBFkL7DSAkqbaEvhN4HxNsAr")
///                .unwrap();
/// assert_eq!(format!("{:?}", address), "Address<NetworkUnchecked>(FXBxTzaqSQsBFkL7DSAkqbaEvhN4HxNsAr)");
/// ```
///
/// ```
/// # use std::str::FromStr;
...
/// let address: Address<NetworkChecked> = Address::from_str("FXBxTzaqSQsBFkL7DSAkqbaEvhN4HxNsAr")
///                .unwrap().assume_checked();
/// assert_eq!(format!("{:?}", address), "FXBxTzaqSQsBFkL7DSAkqbaEvhN4HxNsAr");
/// ```
///
/// ### Relevant BIPs
///
...
#[cfg(feature = "serde")]
crate::serde_utils::serde_string_serialize_impl!(Address, "a Groestlcoin address");
...
impl Address {
    ...
    /// Creates a URI string *groestlcoin:address* optimized to be encoded in QR codes.
    ///
...
impl Address<NetworkUnchecked> {
    ...
    ///
    /// let address: Address<NetworkUnchecked> = "2N83imGV3gPwBzKJQvWJ7cRUY2SpUyKFNVU".parse().unwrap();
...
impl FromStr for Address<NetworkUnchecked> {
    ...
    fn from_str(s: &str) -> Result<Address<NetworkUnchecked>, Error> {
        ...
        let bech32_network = match find_bech32_prefix(s) {
            ...
            "tgrs" | "TGRS" => Some(Network::Testnet), // this may also be signet
        ...
        let (network, payload) = match data[0] {
            PUBKEY_ADDRESS_PREFIX_MAIN =>
                (Network::Groestlcoin, Payload::PubkeyHash(PubkeyHash::from_slice(&data[1..]).unwrap())),
            SCRIPT_ADDRESS_PREFIX_MAIN =>
                (Network::Groestlcoin, Payload::ScriptHash(ScriptHash::from_slice(&data[1..]).unwrap())),
...
mod tests {
    ...
    fn test_p2pkh_address_58() {
        let addr = Address::new(Groestlcoin, Payload::PubkeyHash("162c5ea71c0b23f5b9022ef047c4a86470a5b070".parse().unwrap()));

        assert_eq!(
            addr.script_pubkey(),
            ScriptBuf::from_hex("76a914162c5ea71c0b23f5b9022ef047c4a86470a5b07088ac").unwrap()
        );
        assert_eq!(&addr.to_string(), "FXBxTzaqSQsBFkL7DSAkqbaEvhN4HxNsAr");
    ...
    fn test_p2pkh_from_key() {
        ...
        assert_eq!(&addr.to_string(), "mqkhEMH6NCeYjFybv7pvFC22MFeaH9jnTv");
        assert_eq!(addr.address_type(), Some(AddressType::P2pkh));
        roundtrips(&addr);
    }
    ...
    fn test_p2sh_address_58() {
        let addr = Address::new(Groestlcoin, Payload::ScriptHash("162c5ea71c0b23f5b9022ef047c4a86470a5b070".parse().unwrap()));

        assert_eq!(
            addr.script_pubkey(),
            ScriptBuf::from_hex("a914162c5ea71c0b23f5b9022ef047c4a86470a5b07087").unwrap(),
        );
        assert_eq!(&addr.to_string(), "33iFwdLuRpW1uK1RTRqsoi8rR4NpChEu32");
    ...
    fn test_p2shwsh() {
        ...
        let addr = Address::p2shwsh(&script, Groestlcoin);
    ...
    fn test_address_debug() {
        ...
        let addr_str = "33iFwdLuRpW1uK1RTRqsoi8rR4NpChEu32";
    ...
    fn test_address_type() {
        let addresses = [
            ...
            // ("grs1pw508d6qejxtdg4y5r3zarvary0c5xw7kw508d6qejxtdg4y5r3zarvary0c5xw7kt5nd6y", None),
            // segwit v2
            // ("grs1zw508d6qejxtdg4y5r3zarvaryvaxxpcs", None),
        ];
    ...
    fn test_json_serialize() {
        ...
        let addr = Address::from_str("FXBxTzaqSQsBFkL7DSAkqbaEvhN4HxNsAr").unwrap().assume_checked();
        let json = serde_json::to_value(&addr).unwrap();
        assert_eq!(
            json,
            serde_json::Value::String("FXBxTzaqSQsBFkL7DSAkqbaEvhN4HxNsAr".to_owned())
        );
        ...
        let addr = Address::from_str("33iFwdLuRpW1uK1RTRqsoi8rR4NpChEu32").unwrap().assume_checked();
        let json = serde_json::to_value(&addr).unwrap();
        assert_eq!(
            json,
            serde_json::Value::String("33iFwdLuRpW1uK1RTRqsoi8rR4NpChEu32".to_owned())
        );
        let into: Address = serde_json::from_value::<Address<_>>(json).unwrap().assume_checked();
        assert_eq!(addr.to_string(), into.to_string());
        assert_eq!(
        ...
        let addr = Address::from_str("grs1qyqzpfxy8mv2qhjm3vlaz955d8zapw55vrn5d2h").unwrap().assume_checked();
        let json = serde_json::to_value(&addr).unwrap();
        assert_eq!(
            json,
        ...
        assert_eq!(
            into.script_pubkey(),
            ScriptBuf::from_hex("00142004149887db140bcb7167fa22d28d38ba17528c").unwrap()
        );
    }
    ...
    fn test_qr_string() {
        ...
        {
            let addr = Address::from_str(el).unwrap().require_network(Network::Groestlcoin).expect("mainnet");
            assert_eq!(addr.to_qr_uri(), format!("groestlcoin:{}", el));
        }

        for el in [
            "grs1qyqzpfxy8mv2qhjm3vlaz955d8zapw55vrn5d2h",
            "grs1qwqdg6squsna38e46795at95yu9atm8azzmyvckulcc7kytlcckxsphnwkk",
        ]
        .iter()
        {
    ...
    fn test_valid_networks() {
        ...
        const LEGACY_EQUIVALENCE_CLASSES: &[&[Network]] =
            &[&[Network::Groestlcoin], &[Network::Testnet, Network::Regtest, Network::Signet]];
        const SEGWIT_EQUIVALENCE_CLASSES: &[&[Network]] =
            &[&[Network::Groestlcoin], &[Network::Regtest], &[Network::Testnet, Network::Signet]];
    ...
    fn test_is_related_to_pubkey_p2tr() {
        ...
        assert_eq!(
            address,
            Address::from_str("grs1pgllnmtxs0g058qz7c6qgaqq4qknwrqj9z7rqn9e2dzhmcfmhlu4sxxj9ma")
                .expect("address")
                .require_network(Network::Groestlcoin)
                .expect("mainnet")
        );
    ...
    fn test_fail_address_from_script() {
        ...
        assert_eq!(Address::from_script(&bad_p2wpkh, Network::Groestlcoin), expected);
        assert_eq!(Address::from_script(&bad_p2wsh, Network::Groestlcoin), expected);
        assert_eq!(
            Address::from_script(&invalid_segwitv0_script, Network::Groestlcoin),
