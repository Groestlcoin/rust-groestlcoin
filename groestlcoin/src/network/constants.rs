...
impl Network {
    ...
    ///
    /// ```rust
    /// use groestlcoin::network::constants::{Network, Magic};
...
impl Magic {
    /// Groestlcoin mainnet network magic bytes.
    pub const GROESTLCOIN: Self = Self([0xF9, 0xBE, 0xB4, 0xD4]);
    /// Groestlcoin testnet network magic bytes.
    pub const TESTNET: Self = Self([0x0B, 0x11, 0x09, 0x07]);
    /// Groestlcoin signet network magic bytes.
    pub const SIGNET: Self = Self([0x6F, 0x89, 0x2B, 0x8F]);
    /// Groestlcoin regtest network magic bytes.
...
impl From<Network> for Magic {
    fn from(network: Network) -> Magic {
        match network {
            ...
            Network::Groestlcoin => Magic::GROESTLCOIN,
...
impl TryFrom<Magic> for Network {
    ...
    fn try_from(magic: Magic) -> Result<Self, Self::Error> {
        match magic {
            ...
            Magic::GROESTLCOIN => Ok(Network::Groestlcoin),
...
impl fmt::Display for Magic {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        groestlcoin_internals::fmt_hex_exact!(f, 4, &self.0, groestlcoin_internals::hex::Case::Lower)?;
        Ok(())
    }
}
...
impl fmt::UpperHex for Magic {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        groestlcoin_internals::fmt_hex_exact!(f, 4, &self.0, groestlcoin_internals::hex::Case::Upper)?;
        Ok(())
    }
}
...
mod tests {
    ...
    fn serde_roundtrip() {
        use Network::*;
        let tests = vec![(Groestlcoin, "groestlcoin"), (Testnet, "testnet"), (Signet, "signet"), (Regtest, "regtest")];
    ...
    fn magic_from_str() {
        let known_network_magic_strs = [
            ("f9beb4d4", Network::Groestlcoin),
            ("0b110907", Network::Testnet),
            ("fabfb5da", Network::Regtest),
            ("6f892b8f", Network::Signet),
        ];
