...
//! Groestlcoin network messages.
//!
//! This module defines the `NetworkMessage` and `RawNetworkMessage` types that
//! are used for (de)serializing Groestlcoin objects for transmission on the network.
//!
...
mod test {
    ...
    fn full_round_ser_der_raw_network_message_test() {
        ...
        let msgs = vec![
            ...
            NetworkMessage::GetBlocks(GetBlocksMessage::new(
                vec![groestl_hash([1u8; 32]).into(), groestl_hash([4u8; 32]).into()],
                groestl_hash([5u8; 32]).into(),
            )),
            NetworkMessage::GetHeaders(GetHeadersMessage::new(
                vec![groestl_hash([10u8; 32]).into(), groestl_hash([40u8; 32]).into()],
                groestl_hash([50u8; 32]).into(),
            )),
            NetworkMessage::MemPool,
            ...
            NetworkMessage::FilterAdd(FilterAdd { data: groestl_hash([29u8; 32]).as_byte_array().to_vec() }),
            NetworkMessage::FilterClear,
            NetworkMessage::GetCFilters(GetCFilters {
                filter_type: 2,
                start_height: 52,
                stop_hash: groestl_hash([42u8; 32]).into(),
            }),
            NetworkMessage::CFilter(CFilter {
                filter_type: 7,
                block_hash: groestl_hash([25u8; 32]).into(),
                filter: vec![1, 2, 3],
            }),
            NetworkMessage::GetCFHeaders(GetCFHeaders {
                filter_type: 4,
                start_height: 102,
                stop_hash: groestl_hash([47u8; 32]).into(),
            }),
            ...
            NetworkMessage::GetCFCheckpt(GetCFCheckpt {
                filter_type: 17,
                stop_hash: groestl_hash([25u8; 32]).into(),
            }),
            ...
            NetworkMessage::GetBlockTxn(GetBlockTxn {
                txs_request: BlockTransactionsRequest {
                    block_hash: groestl_hash([11u8; 32]).into(),
    ...
    fn serialize_verack_test() {
        assert_eq!(serialize(&RawNetworkMessage { magic: Magic::from(Network::Groestlcoin), payload: NetworkMessage::Verack }),
    ...
    fn serialize_ping_test() {
        assert_eq!(serialize(&RawNetworkMessage { magic: Magic::from(Network::Groestlcoin), payload: NetworkMessage::Ping(100) }),
    ...
    fn serialize_mempool_test() {
        assert_eq!(serialize(&RawNetworkMessage { magic: Magic::from(Network::Groestlcoin), payload: NetworkMessage::MemPool }),
    ...
    fn serialize_getaddr_test() {
        assert_eq!(serialize(&RawNetworkMessage { magic: Magic::from(Network::Groestlcoin), payload: NetworkMessage::GetAddr }),
    ...
    fn deserialize_getaddr_test() {
        #[rustfmt::skip]
        let msg = deserialize(&[
            0xf9, 0xbe, 0xb4, 0xd4, 0x67, 0x65, 0x74, 0x61,
            0x64, 0x64, 0x72, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0xfd, 0xfb, 0x14, 0xd3
        ]);
        let preimage = RawNetworkMessage {
            magic: Magic::from(Network::Groestlcoin),
