...
pub mod hex {
    ...
    use groestlcoin_internals as internals;
    ...
    #[derive(Debug)]
    pub struct DecodeInitError(groestlcoin_hashes::hex::Error);

    /// Error returned when a hex string contains invalid characters.
    #[derive(Debug)]
    pub struct DecodeError(groestlcoin_hashes::hex::Error);

    /// Hex decoder state.
    pub struct Decoder<'a>(groestlcoin_hashes::hex::HexIterator<'a>);

    impl<'a> Decoder<'a> {
        fn new(s: &'a str) -> Result<Self, DecodeInitError> {
            match groestlcoin_hashes::hex::HexIterator::new(s) {
    ...
    impl super::IntoDeError for DecodeInitError {
        fn into_de_error<E: serde::de::Error>(self) -> E {
            use groestlcoin_hashes::hex::Error;
