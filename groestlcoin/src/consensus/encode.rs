...
impl CheckedData {
    ...
    pub fn new(data: Vec<u8>) -> Self {
        let checksum = groestl2_checksum(&data);
        Self { data, checksum }
    }
...
/// Does a double-GROESTL512 on `data` and returns the first 4 bytes.
...
impl Decodable for CheckedData {
    ...
    fn consensus_decode_from_finite_reader<R: io::Read + ?Sized>(r: &mut R) -> Result<Self, Error> {
        ...
        let expected_checksum = groestl2_checksum(&data);
...
mod tests {
    ...
    fn deserialize_checkeddata_test() {
        let cd: Result<CheckedData, _> =
            deserialize(&[5u8, 0, 0, 0, 134, 30, 66, 180, 1, 2, 3, 4, 5]);
