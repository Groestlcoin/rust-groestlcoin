...
impl Work {
    ...
    pub const MAINNET_MIN: Work = Work(U256(0x0000_0fff_ffff_ffff_0000_0000_0000_0000_u128, 0));

    /// Lowest possible work value for Testnet. See comment on [`Params::pow_limit`] for more info.
    pub const TESTNET_MIN: Work = Work(U256(0x0000_00ff_ffff_ffff_0000_0000_0000_0000_u128, 0));
    ...
    pub const REGTEST_MIN: Work = Work(U256(0x00ff_ffff_ffff_ffff_0000_0000_0000_0000_u128, 0));
...
impl Target {
    /// When parsing nBits, Groestlcoin Core converts a negative target threshold into a target of zero.
    ...
    ///
    /// ref: <https://en.bitcoin.it/wiki/Target>
    // In Groestlcoind this is ~(u256)0 >> 32 stored as a floating-point type so it gets truncated, hence
    ...
    /// defined for the Groestlcoin network and `current` is the current [target] for this block. As
...
/// in the original version of Groestlcoin, likely copying an idea from OpenSSL.
///
/// OpenSSL's bignum (BN) type has an encoding, which is even called "compact" as in groestlcoin, which
...
impl U256 {
    ...
    /// 2**256 / (x + 1) == ~x / (x + 1) + 1
    ///
    /// (Equation shamelessly stolen from groestlcoind)
